...
const ProductCard = (props: Props) => {
  const { title, desc, price, images, productId, stock } = props;
  const { addToCart } = useCart();

  const customStyles = {
    itemShapes: StarSvg,
    activeFillColor: "#8b7cd9",
  ...
  return (
    <div className="font-archer  flex flex-col items-center gap-2 group max-w-sm">
      ...
      <span className="text-xl italic text-gray-700 mx-3 text-center">
        {desc}
      </span>
      <div className="w-full">
        <FancyButton
          className={
            "border bg-[#8cd0e3] w-full text-2xl text-black font-semibold font-cera-stencil invisible group-hover:visible"
          }
          onClick={(event: Event) => {
